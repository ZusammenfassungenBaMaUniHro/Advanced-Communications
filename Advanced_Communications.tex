...
Dies ist beispielsweise dann der Fall, wenn ein Router (oder jede Verbindung zu ihm) ausfällt.
...
\begin{table}[h]
	...
	\begin{tabular}{p{2cm}|p{6cm}|p{6cm}}
		...
		\textbf{Proaktiv}  & Geringe Latenz durch Routing und Zustandsinformationen über alle Verbindungen liegen vor & Overhead und Reparatur von Routen hängt von der Aktualisierungshäufigkeit ab \\ \hline
		\textbf{Reaktiv} & Kein Overhead für das Vorausberechnen der Routen                                         & Latenz durch Routenberechnung                                                \\
	\end{tabular}
\end{table}
...
%TODO Routing Protokolle
%OPTIONAL Routing-Protokolle vor MANETs kopieren
...
\end{itemize}
RIPv1 verwendete noch kein CIDR(Classless Inter-Domain Routing), es wurden also keine Informationen über Subnetze versand, was zur Folge hatte, dass alle Netze dieselbe Größe haben mussten.
...
%OPTIONAL Entscheiden, welche Bilder bleiben/weg sollen
%Hello-Pakete
...
Das \textbf{Hello-Protokoll} ist in Open Shortest Path First für den Netzwerkbetrieb ein integraler Bestandteil des gesamten Routingprozesses. Es ist verantwortlich für:
\begin{multicols}{2}
	\begin{itemize}
...
\subsubsection{Random Early Detection / Discard (RED)}
	\begin{itemize}
	\item An bestimmten Punkten im Netzwerk wird überwacht (bspw. zentraler Router). 
...
Mulitcasts bieten sich dann an, wenn es das Ziel ist, mit so geringem Aufwand wie möglich so viele Beteiligte wie möglich zu erreichen. Wenn es bspw. das Ziel ist, dass mehrere Hörer ein Radioprogramm empfangen, gibt es zwei Möglichkeiten:
	\begin{itemize}
...
Zu komplexeren Formen des Multicastings kann es kommen, wenn mehrere Sender existieren:
	\begin{center}
		\includegraphics[width=12cm]{img/komplMultiCast.png}
	\end{center}
	\begin{itemize}
	\item Sendet A an die Gruppe X, muss das Paket in die Netze 1 und 2 weitergeleitet werden. 
...
Wird beispielsweise gesendet, wenn der Empfänger laut den Routing-Tabellen eines Gateways nicht erreichbar ist, z.B. weil die Distanz zum Zielnetzwerk unendlich ist.
...
Die folgenden Kodierungen können durch verschiedene "`Tricks"' verbessert werden:
\begin{itemize}
...
\subsection{RTCP}
	\begin{itemize}
	\item Das RTCP (RealTime Control Protocol)  definiert verschiedene Kontrollnachrichten. Dabei können mehrere Kontrollnachrichten (RTCP Packets) in einem Compound Paket zusammen versendet werden. 
...
Entsprechend enthält dann die Antwort (im Allgemeinen) die Felder \emph{Content-Encoding} und \emph{Content-Type\footnote{Eigentlich für den MIME-Typ, enthält aber auch den charset.}}
...
\section{E-Mail}
Protokolle zum E-Mail-Versand sind POP3, IMAP und SMTP. Diese bauen auf Transportprotokollen auf, wie 
	\begin{itemize}
...
\subsection{Zustände und Kommandos des Servers}
	...
	\item[AUTHORIZATION] - Phase der Authentisierung 
		\begin{description}
		\item[USER] - Übergabe des Benutzernamens 
		\item[PASS] - Übergabe des Passworts (nur nach USER) 
		\item[APOP](Optional) - Übergabe des MD5 Hashes anstatt eines Passworts
	...
	\item[TRANSACTION] - Phase der Datenübertragung 
			...
			\item[RETR]\textbf{msg} - Abholen der Nachricht mit der Nummer msg, wobei das Nachrichtenende mit\\
			 .$<$CR$><$LF$>$ gekennzeichet ist.
...
Wenn ein Sender eine Datei an einen Receiver sendet, um sie daraufhin an einen Empfänger weiterzusenden, gibt es zwei Reaktionsmöglichkeiten zum Forwareding:
	\begin{enumerate}
...
\subsection{Geräte-Integration}	
	...
	\item Einstellen von Geräte- und Netzwerkparametern
	\item Sicherheit 
...
\subsubsection{DHCP}
	...
	DHCP wird zur automatischen Vergabe von IP-Adressen verwendet - und damit ist es mehr als nur eine statische Zuordnung. Es erfolgt eine temporäre (dynamische) Zuweisung der Adressen, was DHCP zu einem aufwändigerem Protokoll macht. \\
	...
	Außerdem lassen sich bestimmte Parameter anpassen:
		...
		\item[Option Overload] - Die Felder Server Hostname und Boot Filename können für Optionen genutzt werden.
		\item[Server Identifier] - Erlaubt die Unterscheidung zwischen verschiedenen Servern.
	...
	Die wichtigsten DHCP-Vorgänge sind:
		...
		\item[Rückgabe der Adresse ] - Adresse wird vom Client nicht mehr benötigt, daher Information an den Server, dass Adresse zur Wiederverwendung freigegeben ist. 
		\end{description}
...
\subsection{Dienste-Integration}	
	...
	\subsubsection{UPnP}
	UPnP (Universal Plug and Play) vereint Geräte- und \textbf{Dienstekonfiguration} auf der Basis von bewährter Technologie als offene Service-Architektur. Es definiert netzwerkseitige Interaktion, die tatsächliche Implementierung ist herstellerspezifisch.\\
...
\subsection{Herausforderungen}
\textbf{Files}\\
Das Ziel ist ein Zugriff auf Verzeichnisstrukturen über ein Netzwerk, als wären sie lokal. Wünschenswert hierbei sind: Transaktionskontrolle auf verteilten Ressourcen, Fehlermeldungen, Rechtekontrolle, Sicherheit. \\
...
\textbf{Arbeitsweise}
	...
	\item Client führt Dateioperationen auf dem Handle aus
	\end{itemize}
NFS kann optimiert werden indem mehreren Requests und Responses zusammengefasst werden. (Beispielsweise LOOKUP, OPEN, READ) Operationen können mit AND oder OR verknüpft werden und die Auswertung erfolgt in vorgegebener Reihenfolge.\\
...
% 320
\subsection{Telnet}
Telnet wird genutzt, um eine allgemeine, bidirektionale, 8-bit-orientierte Kommunikation zu ermöglichen und so mit entfernten Terminals und terminal-orientierten Prozessen zu kommunizieren.
...
\noindent\textbf{Schema:}
	...
	\item Von welchem Typ/welcher Syntax jedes Attribut besitzt
...
Hierbei handelt es sich gewisser Maßen um einen Akt des Vandalismus. Jemand versucht die Kommunikation zwischen Server und Client zu stören bzw. zu blockieren. Dieser Angriff ist meist schwer zu vereiteln.
...
\noindent Kommunikationsinstanzen
	...
	\begin{enumerate}
	\item \textbf{Authentifikation}: Der Client erfragt Zugriff auf eine Netzwerk Ressource via NAS. Der NAS sendet den RADIUS access request zu einem RADIUS Server. Es können verschiedene Authentifikationsschemata verwendet werden (PAP, CHAP, EAP
	...
	\end{enumerate}
\subsection{PPP}
PPP (Point-to-Point Protocol) ist auf dem Data-Link-Layer in der Internetprotokollfamilie angesiedelt. Im folgenden sind Protokolle zur Authentifikation beschrieben, die für PPP eingesetzt werden.
\subsubsection{CHAP}
%OPTIONAL RFC 1994
...
\subsubsection{Ablauf}
	...
	\item Kollisionen sind möglich, dann wird der aktuelle Frame noch vollständig gesendet:
		\begin{enumerate}
		\item Station ermittelt Zufallszahl k aus [0,1]
...
Die SNR (Signal-Noise-Ratio) und BNR (Bit-Error-Rate) geben einen Aufschluss über die technische Qualität eines Nutzsignals.
	\begin{itemize}
	\item Bei konstantem SNR bewirkt der Einsatz einer höheren Bitübertragungsrate eine höhere BER. 
...
\end{enumerate}
%\begin{itemize}
%	\item CSMA – Station wartet, bis Kanal frei ist.
%	\item CA – Vermeidung von Kollisionen durch verschiedene Wartezeiten bei der Aussendung und RTS/CTS.
%	\item Im Gegensatz zu Ethernet (802.3) erfolgt keine Kollisionserkennung und demnach auch kein Abbruch der Aussendung.
%\end{itemize}
	...
	\begin{itemize}
		...
		\item Zustandsbehaftete und zustandslose Protokolle
			\begin{itemize}
				...
				\item Ein Zustand merkt sich die "`Vorgeschichte"' und damit was passiert ist. Somit ist ein Login immer zustandsbehaftet.
...
\newglossaryentry{dsss}{name=DSSS,description={Direct Sequence Spread Spectrum}}
